module Make (E : Set.OrderedType) = struct
  ...
  and finalize_op t s =
    ...
    | Node (Attempt_add (attempt, root, expected), height, left, pivot, right) ->
      begin
        ...
        | Unknown ->
          let result = if Atomic.get root == expected then Success else Failure in
    ...
    | Node (Removing, _, left, pivot_to_remove, right) ->
      ...
      begin
        ...
        | Node (Dead, _, left_left, left_pivot, left_right) ->
          let expected_height = 1 + max (height left_right) (height right) in
          let s_t_new =
            Node (Removing, expected_height, left_right, pivot_to_remove, right)
          in
          let t_new = Atomic.make s_t_new in
          let expected_height = 1 + max expected_height (height left_left) in
          let s_new = Node (Alive, expected_height, left_left, left_pivot, t_new) in
        ...
        | Node (Alive, left_height, left_left, left_pivot, left_right) ->
          let s_left_dead = Node (Dead, left_height, left_left, left_pivot, left_right) in
  ...
  and push_down_left_locked t s right =
    ...
    then Ok
      ...
      match s_right with
      | Leaf Dead -> if Atomic.compare_and_set t s (Leaf Alive) then Overflow else Ok
      | Leaf Alive ->
  ...
  and balance t =
    ...
    | Node (Balancing_left (s_left, s_left_dead), _, left, pivot, right) ->
      if Atomic.compare_and_set left s_left s_left_dead || Atomic.get left == s_left_dead
      then begin
        match s_left with
        | Node (Alive, _, a, b, c) ->
        ...
        let (_ : bool) = Atomic.compare_and_set t s (s_create left pivot right) in
        Ok
      end
    | Node
      ...
      then begin
        ...
        | Node (Alive, _, a, b, c) ->
          ...
          then begin
            match s_center with
            | Node (Alive, _, c, d, e) ->
              let repr = s_create (create a b c) d (create e pivot right) in
              if Atomic.compare_and_set t s repr then Overflow else Ok
            | _ -> assert false
          end
          else begin
            let s_left_new = s_create a b c in
            let (_ : bool) = Atomic.compare_and_set left s_left_dead s_left_new in
            let (_ : bool) = Atomic.compare_and_set t s (s_create left pivot right) in
            Ok
          end
        | _ -> assert false
      ...
      then begin
        ...
        | Node (Alive, _, x, y, z) ->
          ...
          then begin
            match s_center with
            | Node (Alive, _, v, w, x) ->
              let repr = s_create (create left pivot v) w (create x y z) in
              if Atomic.compare_and_set t s repr then Overflow else Ok
            | _ -> assert false
          end
          else begin
            let s_right_new = s_create x y z in
            let (_ : bool) = Atomic.compare_and_set right s_right_dead s_right_new in
            let (_ : bool) = Atomic.compare_and_set t s (s_create left pivot right) in
            Ok
          end
        | _ -> assert false
    ...
    | Node (Alive, ht, left, pivot, right) ->
      ...
      then begin
        ...
        | Node (Alive, hl, left_left, left_pivot, left_right) ->
          ...
          then begin
            let s_left_dead = Node (Dead, hl, left_left, left_pivot, left_right) in
            let plan = Balancing_left (s_left, s_left_dead) in
            if Atomic.compare_and_set t s (Node (plan, ht, left, pivot, right))
            then balance t
            else Ok
          end
          else begin
      ...
      then begin
        ...
        | Node (Alive, hr, right_left, right_pivot, right_right) ->
          ...
          then begin
            let s_right_dead = Node (Dead, hr, right_left, right_pivot, right_right) in
  ...
  let rec snapshot t =
    let root = Atomic.get t in
    if Atomic.compare_and_set t root (Atomic.make (Copy root)) then root else snapshot t
